...
// Tukuyin kung anong page ang active sa view (default ay 'leaves')
$current_page = isset($_GET['page']) ? $_GET['page'] : 'leaves';

// ==========================================
// 2. BACKEND ACTION HANDLERS (HR/SUPERVISOR NODE)
// ==========================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hr_action'])) {
    ...
    // I-set sa 'Rejected' para hindi na makita sa listahan
...
// 3. DATA QUERIES
// ==========================================
// Kukunin ang mga requests na 'Pending' OR 'Pending HR'
$sql = "SELECT lr.*, e.full_name, e.department 
        FROM leave_requests lr 
        JOIN employees e ON lr.employee_id = e.id 
        WHERE lr.status = 'Pending' OR lr.status = 'Pending HR' 
        ORDER BY lr.id DESC";

$hr_leave_queue = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-zinc-100 font-sans antialiased h-screen overflow-hidden">
   ...
   <?php include 'sidebar.php'; ?>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm text-zinc-700 divide-y divide-zinc-200">
                            <?php if (!$hr_leave_queue || $hr_leave_queue->num_rows == 0): ?>
                                <tr>
                                    <td colspan="5" class="p-12 text-center text-zinc-400 font-medium">Clear! No leave requests pending for HR screening evaluation.</td>
                                </tr>
                            <?php else: ?>
                                    ...
                                    <tr class="hover:bg-zinc-50/50 transition-colors">
                                        ...
                                        <td class="p-4 text-center">
                                            <div class="flex justify-center gap-2">
                                                <button onclick="triggerAction(<?= $req['id'] ?>, 'hr_approve')" class="px-3 py-1.5 bg-blue-500  text-white font-bold text-xs rounded-lg transition-all flex items-center gap-1">
    ...
    // 4. Global Function 
    // This must be outside DOMContentLoaded to be accessible via onclick
    ...
    function highlightActiveSidebarLink() {
        ...
        // Auto-refresh ang page kada 30 segundo (30,000 milliseconds)
setInterval(function() {
    // I-check muna kung walang active na SweetAlert bago mag-refresh
    if (document.querySelector('.swal2-container') === null) {
        location.reload();
    }
}, 30000);
