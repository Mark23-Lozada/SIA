...
<div id="changePasswordModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl border border-amber-100 animate-fade-in">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-black text-zinc-800 flex items-center gap-2"><i class="bi bi-shield-lock-fill text-amber-600"></i> Change Password</h3>
        ...
        <form method="POST" action="" class="space-y-4" onsubmit="return validatePasswordMatch()">
            <div>
                ...
                <div class="relative">
                    <input type="password" id="new_password" name="new_password" required placeholder="Enter new password..." class="w-full bg-amber-50/40 border border-amber-100 rounded-xl p-3 pr-10 text-sm font-semibold text-zinc-700 focus:outline-amber-500">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" id="confirm_password" name="confirm_password" required placeholder="Confirm new password..." class="w-full bg-amber-50/40 border border-amber-100 rounded-xl p-3 pr-10 text-sm font-semibold text-zinc-700 focus:outline-amber-500">
            ...
            <button type="submit" name="submit_change_password" class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-amber-500/20 text-sm">
                Update Password
            </button>
        </form>
    </div>
    ...
    <div class="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl border border-amber-100 animate-fade-in">
        ...
        <form method="POST" action="" class="space-y-4">
            <div>
                ...
                <select name="leave_type" required class="w-full bg-amber-50/40 border border-amber-100 rounded-xl p-3 text-sm font-semibold text-zinc-700 focus:outline-amber-500">
            ...
            <div>
                ...
                <textarea name="reason" rows="3" required placeholder="State your reason for absence..." class="w-full bg-amber-50/40 border border-amber-100 rounded-xl p-3 text-sm font-semibold text-zinc-700 focus:outline-amber-500"></textarea>
            </div>
            <button type="submit" name="submit_leave" class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-amber-500/20 text-sm">
    ...
    <div class="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl border border-amber-100 animate-fade-in">
        ...
        <form method="POST" action="" class="space-y-4" onsubmit="return validateAdvanceAmount()">
            <div>
                ...
                <input type="number" step="0.01" id="adv_amount_input" name="amount" min="1000" max="10000" required placeholder="Enter amount (1,000 - 10,000)..." class="w-full bg-amber-50/40 border border-amber-100 rounded-xl p-3 text-sm font-semibold text-zinc-700 focus:outline-amber-500">
            </div>
            <div>
                <label class="block text-xs font-bold text-zinc-500 uppercase mb-1">Reason Statement</label>
                <textarea name="reason" rows="3" required placeholder="State the reason for advance..." class="w-full bg-amber-50/40 border border-amber-100 rounded-xl p-3 text-sm font-semibold text-zinc-700 focus:outline-amber-500"></textarea>
            </div>
...
<div id="attendanceModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl border border-amber-100 text-center animate-fade-in">
        <div class="flex justify-between items-center mb-4">
...
<div id="payslipModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-amber-100 flex flex-col overflow-hidden animate-fade-in">
        <div class="flex justify-between items-center bg-amber-50/50 px-6 py-4 border-b border-amber-100 no-print">
            <h5 class="modal-title font-bold text-amber-900 flex items-center gap-2 text-sm">
        ...
        <div class="p-6 overflow-y-auto max-h-[75vh]" id="printArea">
            <div class="border border-amber-100 p-6 bg-white rounded-xl text-gray-800 text-xs shadow-sm">
                <div class="text-center border-b pb-4 mb-4">
                    ...
                    <div class="mt-2 inline-block bg-amber-50 text-amber-800 font-mono text-[11px] font-bold px-3 py-1 rounded border border-amber-100">
                        OFFICIAL PAYSLIP STATEMENT | <?= $cutOffPeriod ?? '' ?>
                    </div>
                </div>
                
                <div class="grid grid-cols-2 gap-4 mb-4 border-b pb-4 bg-amber-50/30 p-3 rounded-lg">
                   <div>
                   ...
                   <div>
                    ...
                    <p class="mb-1"><span class="text-gray-500 uppercase font-semibold">Employment Type:</span> <span class="font-semibold text-amber-600">Regular</span></p>
                ...
                <div class="grid grid-cols-2 gap-6 items-start mb-4">
                    <div>
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="flex justify-between py-1.5 font-bold text-gray-900 bg-amber-50/50 px-2 rounded mt-1">
                                <span>Gross Pay (Period)</span> 
                                <span class="font-mono text-amber-700">₱<?= number_format($kinsenas_gross ?? 0, 2) ?></span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-amber-50/40 p-3 rounded-lg mb-4 text-[11px] grid grid-cols-2 gap-2 text-gray-700 border border-amber-100">
                ...
                <div class="bg-amber-950 text-white p-4 rounded-xl flex justify-between items-center shadow-inner">
                    <div>
                        <h4 class="text-[10px] uppercase tracking-widest text-amber-300">Net Pay for this Period</h4>
                        <p class="text-[10px] text-amber-400">Semi-Monthly Payout (15-Day Cycle)</p>
                    </div>
                    <div class="text-right">
                        <h2 class="text-2xl font-black text-amber-300 font-mono">₱<?= number_format($kinsenas_net ?? 0, 2) ?></h2>
                    </div>
                </div>
            </div>
        </div>

        <div class="bg-amber-50/30 px-6 py-3 border-t border-amber-100 flex justify-end gap-2 no-print">
            <button onclick="closePayslipModal()" class="px-4 py-2 bg-white border border-amber-200 hover:bg-amber-50 rounded-xl text-xs font-semibold text-gray-700">Close</button>
            <button onclick="window.print()" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5">
