...
<body class="min-h-screen font-sans antialiased flex flex-col selection:bg-amber-500 selection:text-white">
    ...
    <header class="bg-white/80 backdrop-blur-xl border-b border-slate-100 py-4 px-6 sm:px-12 flex justify-between items-center sticky top-0 z-50 transition-all">
        <div class="flex items-center gap-3.5">
            <div class="w-15 h-15 rounded-2xl bg-gradient-to-tr from-amber-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-amber-500/25">
        ...
        <!-- Apply Now Button -->
        <button onclick="openApplicationModal()" class="bg-amber-500 text-white text-xs sm:text-sm font-extrabold px-6 py-2.5 rounded-xl shadow-lg shadow-amber-500/25 flex items-center gap-2 cursor-pointer transition-all transform active:scale-95">
    ...
    <main class="flex-1 flex items-center justify-center p-4 sm:p-6">
        <div class="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl shadow-slate-200/60 border border-slate-100 overflow-hidden flex flex-col animate-fade-in">
            ...
            <!-- Hero Banner -->
            <div class="bg-amber-500 p-8 sm:p-12 text-center text-white relative overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12">
                ...
                <div class="md:col-span-7 p-6 sm:p-8 flex flex-col justify-between bg-slate-50/50">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-xs font-black text-amber-500 uppercase tracking-widest flex items-center gap-1.5">
                        ...
                        <ul id="job-checklist" class="space-y-3">
                            <div class="py-8 text-slate-400 text-xs font-medium flex items-center justify-center gap-2">
                                <i class="bi bi-arrow-clockwise animate-spin text-amber-500 text-base"></i> Retrieving positions...
                            </div>
                        </ul>
                    </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                        ...
                        <a href="mailto:careers@pannakoda.com" class="font-bold text-amber-500 hover:underline">Contact HR Support</a>
                    </div>
                </div>
                ...
                <div class="md:col-span-5 p-6 sm:p-8 flex flex-col items-center justify-center border-t md:border-t-0 md:border-l border-slate-100 bg-white relative overflow-hidden min-h-[260px]">
                    ...
                    <div class="relative z-10 text-center space-y-3">
                        <div class="inline-flex items-center justify-center w-200 h-200 rounded-2xl bg-gradient-to-tr from-purple-50 to-indigo-50 text-amber-500 text-3xl shadow-inner shadow-amber-500/10 border border-purple-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-amber-500 max-w-[210px] mx-auto mt-1 leading-relaxed">Submit your resume today and jumpstart your professional journey with us.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
                    ...
                    if (activeJobs.length === 0) {
                        listContainer.innerHTML = `
                            <div class="bg-white p-6 rounded-2xl border border-slate-100 text-center shadow-xs">
                                <i class="bi bi-info-circle text-amber-500 text-2xl mb-1.5 block"></i>
                    ...
                    activeJobs.forEach(job => {
                        const li = `
                            <li class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-xs hover:border-purple-300 hover:shadow-md hover:bg-purple-50/10 transition-all group cursor-pointer">
                                <div class="flex items-center gap-3.5">
                                    <div class="w-9 h-9 rounded-xl bg-purple-50 text-amber-500 flex items-center justify-center text-sm shadow-xs border border-purple-100 group-hover:bg-amber-500 group-hover:text-white transition-all">
                                        <i class="bi bi-briefcase-fill"></i>
                                    </div>
                                    <div>
                                        <span class="font-bold text-slate-800 text-xs sm:text-sm group-hover:text-amber-500 transition-colors block">${job.department}</span>
                                        <button type="button" onclick="viewRequirements(${job.id})" class="text-[10px] font-bold text-amber-500 hover:text-purple-700 hover:underline mt-0.5 inline-flex items-center gap-1 bg-transparent border-0 p-0 cursor-pointer">
                                            <i class="bi bi-file-earmark-text"></i> View Requirements
                                        </button>
                                    </div>
                                </div>
                                <span class="text-xs font-black bg-purple-50 text-amber-500 px-3.5 py-1.5 rounded-xl border border-purple-100 group-hover:bg-amber-500 group-hover:text-white group-hover:border-amber-500 transition-all shadow-2xs">
        ...
        window.viewRequirements = function(id) {
            ...
            Swal.fire({
                html: `
                    <div class="text-left">
                        <span class="text-[11px] font-black uppercase tracking-widest text-amber-500 bg-purple-50 px-3 py-1 rounded-lg border border-purple-100">${job.department}</span>
                        ${salaryText ? `<div class="text-sm font-black text-slate-800 mt-3">${salaryText}</div>` : ''}
                        <h2 class="text-base font-black text-slate-900 mt-4 mb-1.5 flex items-center gap-1.5"><i class="bi bi-card-checklist text-amber-500"></i> Job Requirements</h2>
                        <p class="text-xs sm:text-sm text-amber-500 whitespace-pre-line leading-relaxed bg-slate-50 p-3.5 rounded-2xl border border-slate-100">${reqText}</p>
                        ${notesText ? `
                            <h2 class="text-base font-black text-slate-900 mt-4 mb-1.5 flex items-center gap-1.5"><i class="bi bi-info-circle-fill text-amber-500"></i> Things to Know Before Applying</h2>
                            <p class="text-xs sm:text-sm text-amber-500 whitespace-pre-line leading-relaxed bg-slate-50 p-3.5 rounded-2xl border border-slate-100">${notesText}</p>
                        ` : ''}
                    </div>
                `,
            ...
            Swal.fire({
                html: `
                    <div class="text-left mb-5 pb-3 border-b border-slate-100">
                        <span class="text-[11px] font-black uppercase tracking-widest text-amber-500 bg-purple-50 px-3 py-1 rounded-lg border border-purple-100">Candidate Onboarding</span>
                        <h2 class="text-xl sm:text-2xl font-black text-amber-500 mt-2">Application Registration Form</h2>
                        <p class="text-xs sm:text-sm text-amber-500 mt-0.5">Please fill out all required details accurately to submit your application.</p>
                    </div>
                    ...
                    <form id="clientForm" enctype="multipart/form-data" class="text-left font-sans space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                            ...
                            <div class="space-y-3.5">
                                <div>
                                    <label class="block text-[11px] font-bold text-amber-500 uppercase tracking-wider mb-1.5">Full Name</label>
                                    <input type="text" id="full_name" name="full_name" value="${formDataValues.full_name || ''}" placeholder="e.g. Juan Dela Cruz" required class="w-full text-xs sm:text-sm px-4 py-3 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 font-medium text-slate-700 bg-slate-50/50">
                                </div>

                                <div>
                                    <label class="block text-[11px] font-bold text-amber-500 uppercase tracking-wider mb-1.5">Department</label>
                                    <select id="department" name="department" required class="w-full text-xs sm:text-sm px-4 py-3 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 font-medium text-slate-700 bg-slate-50/50">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-[11px] font-bold text-amber-500 uppercase tracking-wider mb-1.5">Email</label>
                                        <input type="email" id="email" name="email" value="${formDataValues.email || ''}" placeholder="name@example.com" required class="w-full text-xs sm:text-sm px-3.5 py-3 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 font-medium text-slate-700 bg-slate-50/50">
                                    </div>
                                    <div>
                                        <label class="block text-[11px] font-bold text-amber-500 uppercase tracking-wider mb-1.5">Phone</label>
                                        <input type="text" id="phone" name="phone" value="${formDataValues.phone || ''}" placeholder="09123456789" maxlength="11" required class="w-full text-xs sm:text-sm px-3.5 py-3 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 font-medium text-slate-700 bg-slate-50/50">
                                    </div>
                                </div>

                                <div>
                                    <label class="block text-[11px] font-bold text-amber-500 uppercase tracking-wider mb-1.5">Home Address</label>
                                    <textarea id="address" name="address" required rows="2" placeholder="Complete address" class="w-full text-xs sm:text-sm px-4 py-2.5 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 font-medium text-slate-700 resize-none bg-slate-50/50">${formDataValues.address || ''}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3.5 flex flex-col justify-between">
                                ...
                                <div class="bg-slate-50/80 border border-slate-100 rounded-2xl p-4 shadow-2xs">
                                    <h4 class="text-[11px] font-black text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                                        <i class="bi bi-shield-lock-fill text-amber-500 text-sm"></i> Government IDs
                                    </h4>
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            <label class="block text-[10px] font-bold text-amber-500 uppercase mb-1">GSIS BP Number</label>
                                            <input type="text" id="gsis_id" name="gsis_id" value="${formDataValues.gsis_id || ''}" required placeholder="GSIS BP Number" class="w-full text-xs px-3 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 text-slate-700">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-amber-500 uppercase mb-1">SSS ID</label>
                                            <input type="text" id="sss_id" name="sss_id" value="${formDataValues.sss_id || ''}" required placeholder="XX-XXXXXXX-X" class="w-full text-xs px-3 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 text-slate-700">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-amber-500 uppercase mb-1">PhilHealth ID</label>
                                            <input type="text" id="philhealth_id" name="philhealth_id" value="${formDataValues.philhealth_id || ''}" required placeholder="XX-XXXXXXXXX-X" class="w-full text-xs px-3 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 text-slate-700">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-amber-500 uppercase mb-1">Pag-IBIG MID</label>
                                            <input type="text" id="pagibig_id" name="pagibig_id" value="${formDataValues.pagibig_id || ''}" required placeholder="XXXX-XXXX-XXXX" class="w-full text-xs px-3 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500 text-slate-700">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label class="block text-[11px] font-bold text-amber-500 uppercase tracking-wider mb-1.5">Upload Resume / CV</label>
                                    <div id="dropzone" class="border-2 border-dashed border-slate-200 rounded-2xl p-4 text-center bg-slate-50/50 cursor-pointer transition-all flex flex-col items-center justify-center gap-1 hover:border-amber-500 hover:bg-purple-50/20">
                                        <i class="bi bi-cloud-arrow-up-fill text-2xl text-amber-500"></i>
                                        <p class="text-xs sm:text-sm font-bold text-slate-700 drop-text">Drag file here or <span class="text-amber-500 underline">browse</span></p>
                ...
                customClass: {
                    ...
                    confirmButton: 'w-full py-3.5 bg-amber-500 text-white font-extrabold rounded-2xl border-0 cursor-pointer shadow-lg shadow-amber-500/25 text-xs sm:text-sm mb-2.5 mt-4 transition-all transform active:scale-98',
                ...
                didOpen: () => {
                    ...
                    dropzone.addEventListener('dragover', (e) => { e.preventDefault(); dropzone.classList.add('border-amber-500', 'bg-purple-50/20'); });
                    ['dragleave', 'drop'].forEach(event => dropzone.addEventListener(event, () => dropzone.classList.remove('border-amber-500', 'bg-purple-50/20')));
                    ...
                    function syncFileVisualText(file) {
                        if(!file) return;
                        dropText.innerHTML = `<span class="text-amber-500 font-bold flex items-center justify-center gap-1.5"><i class="bi bi-file-earmark-check-fill"></i> ${file.name}</span>`;
                    }
                },
                preConfirm: () => {
                    ...
                    .then(data => {
                        if(data.status === "success") {
                            Swal.fire({ 
                                ...
                                text: data.message,
                                customClass: { confirmButton: 'bg-amber-500 hover:bg-purple-700 text-white px-5 py-2.5 rounded-xl font-bold border-0 cursor-pointer shadow-md text-xs' },
                                buttonsStyling: false
                            });
                        } else {
                            Swal.fire({
